...
public class Renderer extends JFrame{
  ...
  public Renderer(Game g, String title, Board board){
    ...
    for(String dis : alias_diseases){
      ...
      float color_r = rand.nextFloat(),
            color_g = rand.nextFloat(),
            color_b = rand.nextFloat();

      // RAWR - To avoid duplicate colors, it'll be useful to generate all color codes separately,
    ...
    content_area.setLayout(new BoxLayout(content_area, BoxLayout.Y_AXIS));  // By default, elements are displayed vertically
    ...
    this.header = new Header(this.grid.width, this.headerHeight, this.header_playerHeight, this.header_diseaseHeight, this);
  ...
  */
  ...
  /*
   * Refresh renderer
   * If a list of city aliases is specified, then only the cells containing those cities are updated. Else, if a null object is specified, the whole grid is updated
   * Updates player info
   */
  public void refresh(ArrayList<String> city_aliases, ArrayList<String> players) {
	  if(city_aliases == null) {
		  city_aliases = this.g.board.used_cities;
	  }
