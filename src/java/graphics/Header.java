...
public class Header extends graphics.Box {
	...
	public Header(int width, int height, int pHeight, int dHeight, Renderer r) {
		...
		for (Disease d : r.diseases) {
			...
			/*
			 * if(d.erradicated)
			 * disease_color.setBorder(BorderFactory.createStrokeBorder(new
			 * BasicStroke(3.0f), Color.green));
			 * 
			 * else if(!d.cure) disease_color.setBorder(BorderFactory.createStrokeBorder(new
			 * BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 2.0f, new
			 * float[] {2.0f}, 0.0f), Color.red));
			 */
