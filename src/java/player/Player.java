...
*/
public class Player{
	// TODO: remove setters/getters and use variables
  public String alias;
  public City city;
  public Role prole;
  public ArrayList<Card> hand = new ArrayList<Card>(); // Player card hand. Hashtable? (might be useful to map the cards by a unique id)
  ...
  */
  public static ArrayList<String> resolvePlayerOrder(Hashtable<String, Player> players){
	...
	// Target player order & aux hashtable used to identify the highest card of each player
    ...
    for(Player p : players.values()) {
    	if(p.hand == null) {
            if(Options.LOG.ordinal() >= CustomTypes.LogLevel.CRITICAL.ordinal())
                System.out.printf("[Player] CRITICAL: Cannot resolve player order. At least a player have no hand\n");
            
            return null;
    	}
    	
    	// Iterates every card in the player hand to find the city with the highest population
    	...
    	// Evaluates ordered list and checks the highest city population of every sorted player
    	int sorted_index = 0;
